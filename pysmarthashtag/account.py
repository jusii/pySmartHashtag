...
# Default page size for the trackpoints endpoint, matching what the
# official Smart app sends. Across captured probes ``totleSize`` never
# exceeded this cap; the wrapper logs a WARNING if it ever does so a
# future maintainer knows to add pagination here.
...
class SmartAccount:
    ...
    async def get_vehicles(self, force_init: bool = False) -> None:
        ...
        for vin, vehicle in self.vehicles.items():
            ...
            except Exception:  # noqa: BLE001  # Best-effort: any failure must not break refresh.
                _LOGGER.debug(
    ...
    async def get_vehicle_state(self, vin) -> dict:
        ...
        """
        _LOGGER.debug("Getting vehicle state for %s", vin)
    ...
    async def get_trip_trackpoints(
        ...
        * ``code: "8153"`` ("data unavailable" — surfaces as
        ...
        Pagination is intentionally NOT implemented: the official Smart
        app always sends ``pageIndex=0, pageSize=500``, and across
        captured probes ``totleSize`` never exceeded the cap. If a
        future trip ever does, this method logs a WARNING so we know
        to revisit.
        """
